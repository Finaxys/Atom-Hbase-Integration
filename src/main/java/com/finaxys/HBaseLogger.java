...
class HBaseLogger extends Logger {
  ...
  private int flushedRow = 0;

  HashMap<ByteBuffer,Put> rkSet = new HashMap<>();
  ...
  private void putTable(@NotNull Put p) {
    try {
      ByteBuffer rk = ByteBuffer.wrap(p.getRow());
      if(rkSet.containsKey(rk)) {
      LOGGER.log(Level.INFO, "RK : " + Bytes.toString(p.getRow()) +
          " PUT 1 : " + rkSet.get(rk).toJSON() + "\n Put 2 : " + p.toJSON());
      }
      this.rkSet.put(rk,p);
      table.put(p);
      ++stackedPuts;
  ...
  private void flushPuts() {
    try {
      ...
      flushedRow += table.getWriteBuffer().size();
      table.flushCommits();
    } catch (InterruptedIOException e) {
  ...
  public void close() throws Exception {
    ...
    LOGGER.log(Level.INFO, "number of puts = " + countPuts);
    LOGGER.log(Level.INFO, "Internal buffer flushed size = " + flushedRow);
    LOGGER.log(Level.INFO, "Unique rk size = " + rkSet.size());
  ...
  private String createRequired(@NotNull String name) {
    ...
    long rowKey = Long.reverseBytes(test);
